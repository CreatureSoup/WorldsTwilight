...
const ANIM = {
  ...
  bob(t, unit, def) {
    if (unit.noAnim || unit.noBob) return 0;   // noBob: вертикальное «дыхание» отдаём щупальцам (bodyOff.y) — корпус и ноги качаются СИНХРОННО
  ...
  // Вибрация бура вдоль направления взгляда при копании (в px) — «долбёжка», бур выдвигается к породе.
  drillWob(t, unit) { return (unit.noAnim || !unit.drilling) ? 0 : Math.sin(t * 30) * this.R() * 0.28; },
};
