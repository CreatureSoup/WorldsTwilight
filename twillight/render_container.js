...
function _drawCrate(ctx, cx, cy, c, game) {
  ...
  ctx.fillStyle = locked ? '#d0402f' : acc; ctx.globalAlpha = breaching ? 0.7 + 0.3 * Math.sin(t * 22) : 0.9;
  ctx.fillRect(cx - 3, cy - 2, 6, 6); ctx.globalAlpha = 1;
  if (locked) { ctx.strokeStyle = '#d0402f'; ctx.lineWidth = 1.4; ctx.beginPath(); ctx.arc(cx, cy - 3, 4, Math.PI, 0); ctx.stroke(); }   // дужка замка (нет узла)
  if (breaching) {   // КОЛЬЦО-ПРОГРЕСС взлома (сейф-крекинг, янтарь) + дребезг-биты + процент
    const p = c.breach, R = s * 1.7;
    ctx.globalCompositeOperation = 'lighter';
    ctx.strokeStyle = 'rgba(224,176,72,0.3)'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(cx, cy, R, 0, 6.283); ctx.stroke();
    ctx.strokeStyle = '#e0b048'; ctx.lineWidth = 2.4; ctx.beginPath(); ctx.arc(cx, cy, R, -Math.PI / 2, -Math.PI / 2 + 6.283 * p); ctx.stroke();
