...
// чтобы движение читалось как «вздох», а не «прыжок». Состояние (_barPhase/_barPrev/
// _barText/_barT0) ШАГАЕТ city._stepBarText (логика) — здесь ТОЛЬКО чтение и отрисовка.
function _drawBarText(ctx, city, x, y, slide) {
  ...
  if (animating && t < 1) {
    ...
    ctx.fillText(city._barText, x, y);   // _barPrev оставляем как есть: t>=1 → ветка else; следующая смена фазы перезапишет
  }
}
