...
function drawPrintGhost(ctx, game, camera) {
  ...
  if (U) {   // РАДИУС СТРОИТЕЛЬСТВА вокруг юнита — тонкая пунктирная линия (видно границу досягаемости печати)
    ...
    ctx.beginPath(); ctx.arc(ucx, ucy, reach * TILE, 0, TAU); ctx.stroke();
    ctx.restore();
  }
...
function drawWallStruct(ctx, s, cx, cy) {
  ...
  for (const [dx, dy] of [[-1, -1], [1, -1], [-1, 1], [1, 1]]) { ctx.beginPath(); ctx.arc(cx + dx * (h - 4), cy + dy * (h - 4), 1.6, 0, TAU); ctx.fill(); }
}
...
function drawTurretStruct(ctx, s, cx, cy) {
  ...
  if (s.flash > 0) { ctx.fillStyle = rail ? '#dff0ff' : '#ffe0a0'; ctx.beginPath(); ctx.arc(bx, by, rail ? 4 : 3.2, 0, TAU); ctx.fill(); }
...
function drawMwStruct(ctx, s, cx, cy) {
  ...
  ctx.fillStyle = PAL.enemyEye; ctx.beginPath(); ctx.arc(cx, oy - 1, 2, 0, TAU); ctx.fill();
  ctx.restore();
}
...
function drawEmpStruct(ctx, s, cx, cy) {
  ...
  ctx.save(); ctx.globalAlpha *= off;
  ctx.fillStyle = '#1c2e34'; ctx.beginPath(); ctx.arc(cx, cy, r, 0, TAU); ctx.fill();
  ctx.strokeStyle = s.def.color; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.arc(cx, cy, r, 0, TAU); ctx.stroke();
  for (const e of [0.45, 0.75]) { ctx.beginPath(); ctx.ellipse(cx, cy, r * e, r, 0, 0, TAU); ctx.stroke(); }   // катушки
  ctx.fillStyle = s.def.color; ctx.beginPath(); ctx.arc(cx, cy, 2.2, 0, TAU); ctx.fill();
  ctx.restore();
}
...
function drawRepulsorStruct(ctx, s, cx, cy) {
  const r = TILE / 2 - 5, off = s.energy < s.def.eShot ? 0.5 : 1;
  ctx.save(); ctx.globalAlpha *= off;
  ctx.fillStyle = '#2a2236'; ctx.beginPath(); ctx.arc(cx, cy, r, 0, TAU); ctx.fill();
  ctx.strokeStyle = s.def.color; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(cx, cy, r * 0.62, 0, TAU); ctx.stroke();
...
function drawJammerStruct(ctx, s, cx, cy) {
  ...
  ctx.fillStyle = s.def.color; ctx.beginPath(); ctx.arc(cx, cy - r, 2.4, 0, TAU); ctx.fill();
  ctx.restore();
}
...
function drawAura(ctx, cx, cy, radiusTiles, color) {
  ...
  ctx.beginPath(); ctx.arc(cx, cy, radiusTiles * TILE, 0, TAU); ctx.stroke(); ctx.restore();
}
...
function drawSiegeStruct(ctx, s, cx, cy) {
  ...
  ctx.beginPath(); ctx.arc(hx, hy, r * 0.24, 0, TAU); ctx.fill();   // ядро-резонатор
  ctx.restore();
}
...
function drawCourierStruct(ctx, s, cx, cy) {
  ...
  ctx.beginPath(); ctx.ellipse(cx, dy, r * 0.55, r * 0.32, 0, 0, TAU); ctx.fill(); ctx.stroke();
  ctx.strokeStyle = '#2a3a3c'; ctx.lineWidth = 1;
  for (const sx of [-1, 1]) { ctx.beginPath(); ctx.moveTo(cx, dy - r * 0.1); ctx.lineTo(cx + sx * r * 0.7, dy - r * 0.35); ctx.stroke(); ctx.beginPath(); ctx.arc(cx + sx * r * 0.7, dy - r * 0.4, r * 0.22, 0, TAU); ctx.stroke(); }
  // антенна-релей сбоку
  ...
  ctx.fillStyle = col; ctx.globalAlpha *= beat; ctx.beginPath(); ctx.arc(cx + r * 0.95, cy - r * 0.5, 1.8, 0, TAU); ctx.fill();
  ctx.restore();
}
...
function drawStructDeath(ctx, s, cx, cy) {
  ...
  for (let i = 0; i < 5; i++) { const a = (i / 5) * TAU + s.px, l = r * (0.6 + p * 1.4); ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx + Math.cos(a) * l, cy + Math.sin(a) * l); ctx.stroke(); }
  ctx.restore();
}
