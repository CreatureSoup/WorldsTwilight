...
Object.assign(Game.prototype, {
  updateHazards(dt) {
    ...
    for (const r of w.robots) {
      if (r.state === 'buried') {
        if (!r.dug) continue;
        if (defuse) { r.state = 'dead'; r.defused = true; if (!this.debug) this.logEvent('ОСТАНКИ ОБЕЗВРЕЖЕНЫ'); }
        else { r.state = 'wake'; r.t = 0; if (!this.debug) this.logEvent('БОЕВОЙ ПРОТОКОЛ ОЖИЛ'); }
        continue;
      }
      if (r.state === 'wake') {
        r.t += dt;
        ...
        if (r.fired >= ROBOT_SHOTS) {   // отстрелялся → ОСЕДАЕТ (питание гаснет, корпус оплывает) → dead
          ...
          if (!this.debug) this.logEvent('БОЕВОЙ ПРОТОКОЛ ИССЯК');
        }
    ...
    for (const m of w.mines) {
      if (m.state === 'buried') {
        if (!m.dug) continue;
        if (defuse) { m.state = 'done'; m.defused = true; if (!this.debug) this.logEvent('МИНА ОБЕЗВРЕЖЕНА'); }
  ...
  _mineBlast(m) {
    ...
    if (!this.debug) this.logEvent('ВЗРЫВ СТАРОЙ МИНЫ');
  },
});
