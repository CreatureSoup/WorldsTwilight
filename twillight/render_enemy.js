...
// Телеграф-кольцо ЗАМАХА (wind-фаза): мерцающее кольцо сжимается к корпусу — общий хелпер
// охотника/тарана/залежня (был скопирован ×3 — audit_2026-08). f — прогресс замаха 0..1 → радиус.
function _windRing(ctx, cx, cy, radius, blinkHz, alpha, color, lw) {
  const t = performance.now() / 1000;
  ctx.save(); ctx.globalAlpha = alpha * (0.4 + 0.6 * Math.abs(Math.sin(t * blinkHz)));
  ctx.strokeStyle = color; ctx.lineWidth = lw;
  ctx.beginPath(); ctx.arc(cx, cy, radius, 0, TAU); ctx.stroke(); ctx.restore();
}
// Семенящие ножки-стерженьки наземного краулера (собиратель/скверносей — был дубль ×2).
function _scuttleLegs(ctx, cx, cy, r, moving, ph, col, hz, phStep) {
  const t = performance.now() / 1000;
  ctx.strokeStyle = col; ctx.lineWidth = 2;
  for (let i = 0; i < 4; i++) {
    const lx = cx + (i - 1.5) * r * 0.5, sw = moving ? Math.sin(t * hz + ph + i * phStep) * 2 : 0;
...
function drawEnemies(ctx, enemies, camera) {
  ...
  for (const e of enemies) {
    ...
    ctx.fillStyle = e.drilling ? PAL.enemyHot : PAL.enemyEye;
    ctx.beginPath(); ctx.arc(cx + e.dx * r * 0.35, cy + e.dy * r * 0.35, r * 0.3, 0, TAU); ctx.fill();
  }
}
...
function drawEnemyDeath(ctx, e, cx, cy, r) {
  ...
  ctx.beginPath(); ctx.ellipse(0, 0, r * (1 - p * 0.4), r * 0.8 * (1 - p * 0.4), 0, 0, TAU); ctx.fill();
  ...
  for (let i = 0; i < 5; i++) { const a = (i / 5) * TAU + e.seed, l = r * (0.5 + p * 1.6); ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx + Math.cos(a) * l, cy + Math.sin(a) * l); ctx.stroke(); }
  ctx.restore();
}
...
function drawMidge(ctx, e, cx, cy, r) {
  ...
  ctx.beginPath(); ctx.ellipse(cx, cy, rr, rr * 0.85, 0, 0, TAU); ctx.fill();
  ...
  ctx.beginPath(); ctx.arc(cx, cy, rr * 0.34, 0, TAU); ctx.fill();
}
...
function drawMender(ctx, e, cx, cy, r) {
  ...
  if (healing) {
    ...
    ctx.beginPath(); ctx.arc(cx, cy, r * 1.3, 0, TAU); ctx.stroke(); ctx.restore();
  }
  ctx.fillStyle = '#23302a';
  ctx.beginPath(); ctx.ellipse(cx, cy, r * 0.9, r * 0.7, 0, 0, TAU); ctx.fill();
  ...
  ctx.beginPath(); ctx.arc(cx + fx * r * 0.45, cy, r * 0.22, 0, TAU); ctx.fill();
}
...
function drawMortar(ctx, e, cx, cy, r) {
  ...
  ctx.beginPath(); ctx.ellipse(cx, cy, r * 0.95, r * 0.8, 0, 0, TAU); ctx.fill();
  ...
  if (e.firing > 0) { ctx.save(); ctx.globalCompositeOperation = 'lighter'; ctx.fillStyle = 'rgba(255,160,90,0.8)'; ctx.beginPath(); ctx.arc(r * 1.2, 0, r * 0.32, 0, TAU); ctx.fill(); ctx.restore(); }
  ctx.restore();
  ctx.fillStyle = e.firing > 0 ? '#ff9a4a' : PAL.enemyEye;
...
function drawSiegeRam(ctx, e, cx, cy, r) {
  ...
  if (e.cstate === 'wind') _windRing(ctx, cx, cy, r * (1.7 - 0.7 * Math.min(1, (e.cT || 0) / RAM_WIND)), 22, 0.5, '#ff7a3a', 2.5);
  ctx.save(); ctx.translate(cx, cy); ctx.rotate(ang);
  ...
  ctx.restore();
  ctx.fillStyle = charging ? PAL.enemyHot : PAL.enemyEye;
  ctx.beginPath(); ctx.arc(cx + Math.cos(ang) * r * 0.2, cy + Math.sin(ang) * r * 0.2, r * 0.26, 0, TAU); ctx.fill();
}
...
function drawMinePlanter(ctx, e, cx, cy, r) {
  ...
  ctx.beginPath(); ctx.ellipse(cx, cy, r * 0.9, r * 0.72, 0, 0, TAU); ctx.fill();
  ...
  ctx.beginPath(); ctx.arc(cx, cy - r * 0.55, r * 0.42, 0, TAU); ctx.fill();
  ...
  ctx.globalAlpha = lit; ctx.fillStyle = armedBlink ? '#ff5030' : PAL.enemyEye;
  ctx.beginPath(); ctx.arc(cx, cy - r * 0.55, r * 0.18, 0, TAU); ctx.fill(); ctx.restore();
}
...
function drawLurker(ctx, e, cx, cy, r) {
  ...
  if (e.lurkState === 'wind') _windRing(ctx, cx, cy, r * (1.5 - 0.6 * Math.min(1, (e.lurkT || 0) / LURKER_WIND)), 28, 0.5, '#ff6a3a', 2);   // телеграф замаха
  ctx.save(); ctx.translate(cx, cy); ctx.rotate(ang);
  ...
  ctx.fillStyle = hot ? PAL.enemyHot : '#a83a2f';
  ctx.beginPath(); ctx.arc(cx + Math.cos(ang) * r * 0.3, cy + Math.sin(ang) * r * 0.1, r * 0.22, 0, TAU); ctx.fill();
}
...
function drawBlightSower(ctx, e, cx, cy, r) {
  ...
  _scuttleLegs(ctx, cx, cy, r, moving, ph, '#4a3a52', 14, 2);   // ножки семенят
  ctx.fillStyle = '#2c2433';   // сегментный корпус
  ctx.beginPath(); ctx.ellipse(cx, cy + r * 0.1, r * 1.0, r * 0.5, 0, 0, TAU); ctx.fill();
  ...
  ctx.fillStyle = '#b07ad0'; ctx.beginPath(); ctx.arc(cx, cy - r * 0.95, r * 0.2, 0, TAU); ctx.fill(); ctx.restore();
  ctx.fillStyle = PAL.enemyEye;   // красный глаз
  ctx.beginPath(); ctx.arc(cx + fx * r * 0.55, cy + r * 0.05, r * 0.16, 0, TAU); ctx.fill();
}
...
function drawBlightBeacons(ctx, beacons, camera) {
  ...
  for (const b of beacons) {
    ...
    ctx.beginPath(); ctx.arc(cx, cy, BLIGHT_BEACON_R * TILE * (0.5 + 0.06 * pulse), 0, TAU); ctx.stroke();
    ctx.restore();
    // кол-эмиттер
    ...
    ctx.fillStyle = '#c08ae0'; ctx.beginPath(); ctx.arc(cx, cy - 8, 3, 0, TAU); ctx.fill(); ctx.restore();
    // HP-полоса (если бит)
    if (b.hp < b.maxHp) { const w = 16, h = 2.5, x = cx - w / 2, y = cy - 16; ctx.fillStyle = 'rgba(0,0,0,0.6)'; ctx.fillRect(x, y, w, h); ctx.fillStyle = b.hit > 0 ? PAL.enemyHot : '#b07ad0'; ctx.fillRect(x, y, w * Math.max(0, b.hp / b.maxHp), h); }
  }
}
...
function drawCollector(ctx, e, cx, cy, r) {
  ...
  const moving = e.state2 === MOVING;
  _scuttleLegs(ctx, cx, cy, r, moving, ph, '#46563c', 16, 2.1);
  // корпус — широкий и низкий (жук)
  ctx.fillStyle = '#2f3a30';
  ctx.beginPath(); ctx.ellipse(cx, cy + r * 0.08, r * 1.02, r * 0.55, 0, 0, TAU); ctx.fill();
  ...
  ctx.fillStyle = e.drilling ? PAL.enemyHot : PAL.enemyEye;
  ctx.beginPath(); ctx.arc(cx + fx * r * 0.62, cy - r * 0.02, r * 0.13, 0, TAU); ctx.fill();
  ctx.beginPath(); ctx.arc(cx + fx * r * 0.34, cy + r * 0.06, r * 0.13, 0, TAU); ctx.fill();
}
...
function drawHunter(ctx, e, cx, cy, r) {
  ...
  if (!ax && !ay) ax = 1;
  const ang = Math.atan2(ay, ax), charging = e.cstate === 'charge';
  if (e.cstate === 'wind') _windRing(ctx, cx, cy, r * (1.6 - 0.7 * Math.min(1, (e.cT || 0) / HUNTER_WIND)), 26, 0.55, '#ff7a3a', 2);   // телеграф: сжимающееся пульс-кольцо
  ctx.save(); ctx.translate(cx, cy); ctx.rotate(ang);
  ...
  ctx.beginPath(); ctx.arc(r * 0.4, 0, r * 0.3, 0, TAU); ctx.fill();
  ctx.restore();
}
...
function drawHacker(ctx, e, cx, cy, r) {
  ...
  ctx.beginPath();
  for (let i = 0; i < 6; i++) { const a = i / 6 * TAU + 0.52, px = cx + Math.cos(a) * r, py = cy + Math.sin(a) * r * 0.92; i ? ctx.lineTo(px, py) : ctx.moveTo(px, py); }
  ctx.closePath(); ctx.fill();
  ...
  ctx.fillStyle = hacking ? PAL.enemyHot : PAL.enemyEye;               // сканер-глаз — КРАСНЫЙ (общий знак диких)
  ctx.beginPath(); ctx.arc(cx + (e.dx || 0) * r * 0.2, cy + (e.dy || 0) * r * 0.2, r * 0.28, 0, TAU); ctx.fill();
}
...
function drawSniper(ctx, e, cx, cy, r) {
  ...
  if (firing) { ctx.fillStyle = '#e8ff7a'; ctx.beginPath(); ctx.arc(r * 1.85, 0, r * 0.42, 0, TAU); ctx.fill(); }
  ctx.restore();
  ctx.fillStyle = firing ? '#ff8a5a' : PAL.enemyEye;                // оптика-глаз — КРАСНЫЙ (общий знак диких)
...
function drawShots(ctx, shots, camera) {
  ...
  for (const s of shots.list) {
    ...
    ctx.fillStyle = '#e8ff7a'; ctx.beginPath(); ctx.arc(x2, y2, 2, 0, TAU); ctx.fill();
  }
  ctx.restore();
}
...
function drawRaider(ctx, e, cx, cy, r) {
  ...
  ctx.beginPath(); ctx.arc(cx + e.dx * r * 0.25, cy + e.dy * r * 0.25, r * 0.32, 0, TAU); ctx.fill();
  ...
  if (e.draining) {
    ...
    ctx.beginPath(); ctx.arc(cx, cy, r * 1.3, -Math.PI / 2, -Math.PI / 2 + frac * TAU); ctx.stroke();
  ...
  if (e.carry) {
    ...
    ctx.beginPath(); ctx.arc(cx, cy - r - 5, r * 0.42, 0, TAU); ctx.fill();
    ctx.restore();
  }
}
