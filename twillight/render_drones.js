...
const DRONE_COL = { collector: PAL.screwGreen, courier: '#5fd0e0', battery: '#f0c84a', scout: '#7fb0e0', hacker: '#c06ee6' };
...
function _drawDrone(ctx, camera, d) {
  ...
  if (d.kind === 'hacker' && d.state === 'hack' && d.target) {   // канал взлома — луч к сердцу гнезда + кольцо-прогресс
    ...
    ctx.globalAlpha = 0.85; ctx.beginPath(); ctx.arc(hx, hy, TILE * 0.6, -Math.PI / 2, -Math.PI / 2 + f * TAU); ctx.stroke();
    ctx.restore();
  }
  ...
  ctx.beginPath(); ctx.ellipse(x, y + bob, TILE * 0.36, TILE * 0.26, 0, 0, TAU); ctx.fill(); ctx.stroke();
  ...
  ctx.fillStyle = col; ctx.beginPath(); ctx.arc(x, y + bob, 2, 0, TAU); ctx.fill();              // ядро-глаз
  if (d.carry || d.carryMap) { ctx.fillStyle = '#e8dcc0'; ctx.fillRect(x - 2, y + bob + TILE * 0.22, 4, 4); }   // несомый груз
  ctx.restore();

  if (d.fx > 0) { ctx.save(); ctx.globalCompositeOperation = 'lighter'; ctx.globalAlpha = Math.min(1, d.fx * 2); ctx.fillStyle = col; ctx.beginPath(); ctx.arc(x, y + bob, TILE * 0.55 * (1 - d.fx), 0, TAU); ctx.fill(); ctx.restore(); }
}
