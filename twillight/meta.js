...
function _metaBuildGraph() {
  ...
  _repos('print_o0', -28, 975);    // Дешёвые тела (красная, ЗАКРЫТО) — дальше от mast_ech (синий сенсорный край −51)
  ...
  cn('amb_dock',   'mid', 660, 41, 'Ремонтный док', 'чинит юнит на базе', 'stab', 'Открывает трек РЕМОНТНЫЙ ДОК: на базе юнит восстанавливает HP корпуса со временем (4 ур.).');
  cn('amb_nav',    'mid', 672, 58.7, 'Навигация до города', 'путь к базе', 'map', 'Когда времени до базы на текущей скорости впритык к таймеру гибернации — зажигает светящийся путь к городу. Учитывает апгрейды скорости и таймера.');
  cn('amb_predict','out', 860, 58.8, 'Предикт волн', 'прогноз атак', 'detector', 'РЕАЛИЗОВАН: в HUD под «ЦИКЛ N» — графический таймер обратного отсчёта до следующей волны + тип надвигающегося врага (глиф/имя) и тег «НОВАЯ» при появлении нового типа; запись в логе. Без узла виден только прогресс-бар цикла.');
  cn('amb_cont',   'mid', 535, 71, 'Контуры', 'запас контуров', 'body', 'Открывает трек КОНТУРЫ в апгрейдах базы: усиливает контуры энергосбережения города по очереди — внешний → внутренний → ядро.');
  cn('amb_regen',  'out', 715, 76.1, 'Регенерация контуров', 'самовосстановление', 'resonance', 'Контуры энергосбережения города медленно восстанавливают заряд САМИ. Трек АВТО-ПОЧИНКА: уровни расширяют охват ядро → +внутр. → +внешний. (утерянное кольцо НЕ возвращает)');
  cn('amb_recon',  'out', 901, 74.3, 'Реконструкция', 'возврат утерянных', 'ascend', 'Усиливает авто-починку: ВОЗВРАЩАЕТ утерянные (обнулённые) контуры — воскрешает кольцо и регенит его HP тем же темпом, в охвате авто-почини. Заменяет обычную регенерацию.');
  ...
  kn('kart_hub',     'hub', 360, 128, 'Дешифратор', 'скан −30%', 'decode', 'Каркас ветви МИР. РЕАЛИЗОВАН: ускоряет извлечение данных со ВСЕХ источников (серверы/враги/руины/останки) на 30%. От него — 4 линии: ДЕТЕКТОР, ДАННЫЕ, ГОРОДА (через Объём) и ВЗЛОМ.');
  ...
  kn('mast_sr',      'out', 580, 95, 'Детектор загрязнения', 'свойство сканера', 'detector', 'Апгрейд сканера: засекает СТОРОНУ источника загрязнения, пока ты в зоне фона. Отдельного модуля не нужно — просто работает (живые/неточные показания, точнее вблизи).');
  kn('mast_sh',      'out', 790, 95, 'Экран помех', 'модуль в доп-слот', 'resonance', 'Модуль для доп-слота: гасит помехи интерфейса от радиации (база). Ставишь в доп-слот на сборке; трек ЭКРАН ПОМЕХ в апгрейдах базы докручивает. (переехал из ЮНИТ к Детектору)');
  // линия ДАННЫЕ (прямо от Дешифратора): данные из останков → данные из руин
  kn('kart_wreck',   'mid', 585, 113.6, 'Данные из останков', 'скан мёртвых роботов', 'salvage', 'РЕАЛИЗОВАН (см. hazards.js): позволяет СКАНИРОВАТЬ уже мёртвые останки роботов в породе и извлекать из них данные кодекса (как серверы). Вход линии ДАННЫХ.');
  kn('kart_ruins',   'out', 800, 113.5, 'Данные из руин', 'пещеры-сцены', 'relic', 'РЕАЛИЗОВАН: ОТКРЫВАЕТ извлечение данных из руин (пещер-сцен) — без узла объёмный скан невозможен (лог «нет методов»). С узлом — вход в пещеру запускает скан → данные.');
  // ответвление ГОРОДА (Объём данных → пробуждение → взлом города); сходится с ВЗЛОМОМ на hackcity
  kn('kart_data',    'mid', 735, 132.7, 'Объём данных', '×2 данных', 'archive', 'РЕАЛИЗОВАН: каждый источник даёт В 2 РАЗА больше фрагментов данных кодекса (серверы/враги/руины/останки/города/артефакты). Ответвление от Дешифратора; ведёт к ГОРОДАМ.');
  kn('kart_wake',    'out', 950, 131.7, 'Пробуждение города', 'нейтральный город', 'rune', 'РЕАЛИЗОВАН: РАЗРЕШАЕТ взлом спящего нейтрального города (модулем взлома — доп-действие, цифра 2) → пробуждение в автономную дружественную фракцию (копатель+сборщик; турели игрока их не трогают). Часть пути к ПОБЕДЕ.');
  // линия ВЗЛОМ (от Дешифратора): обезвреживание (ОТКРЫВАЕТ модуль взлома) → взлом юнитов (+стелс) → взлом диких
  kn('kart_defuse',  'mid', 580, 143.2, 'Взлом опасных останков', 'модуль взлома', 'decode', 'РЕАЛИЗОВАН: ОТКРЫВАЕТ МОДУЛЬ ВЗЛОМА (доп-слот, доп-действие) + юнит при откопке ОБЕЗВРЕЖИВАЕТ старые мины/останки вместо срабатывания. Первый узел линии ВЗЛОМА.');
  kn('kart_stun',    'out', 815, 145.5, 'Взлом юнитов', 'стан в радиусе', 'resonance', 'По доп-действию (цифра) тормозит вражеские юниты в радиусе. От него ОТВЕТВЛЕНИЕ — Стелс (свой модуль).');
  kn('kart_stealth', 'out', 1075, 151.6, 'Стелс-модуль', 'невидимость', 'obsidian', 'Ответвление от «Взлома юнитов»: открывает СВОЙ модуль — по доп-действию враги на экране «не видят» юнит.');
  kn('kart_jam',     'out', 1035, 141.5, 'Взлом диких городов', 'замедляет волны', 'quiet', 'Замедляет фазы волн врагов диких городов. Вместе с «Пробуждением города» открывает Взлом нейтрального города.');
  // ВЕНЕЦ: КОНВЕРГЕНЦИЯ — требует «Пробуждение города» И «Взлом диких городов» (allDeps)
  kn('kart_hackcity','out', 1205, 137.2, 'Взлом нейтрального города', 'переговоры', 'contact', 'ВЕНЕЦ ветви МИР. РЕАЛИЗОВАН: после пробуждения запускает БОЛЬШОЙ таймер перехвата реактора → кат-сцена передачи ядра юниту → конец сессии ПОБЕДОЙ + директива «Спящий кластер» + бонус МТ. Требует «Пробуждение города» И «Взлом диких городов».');
  nodes.find((x) => x.id === 'kart_hackcity').allDeps = ['kart_wake', 'kart_jam'];   // КОНВЕРГЕНЦИЯ: доступен только при ОБОИХ родителях (metaAvail)
  ['kart_stun', 'kart_stealth', 'kart_jam'].forEach((id) => { const nn = nodes.find((x) => x.id === id); if (nn) nn.wip = true; });   // НЕТ ФУНКЦИОНАЛА → баннер «В РАЗРАБОТКЕ» в карточке (mtRenderCard); убирать флаг по мере реализации
