  ...
  try {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      for (const key of Object.keys(sprites)) {
        // ⚠️ borer:* (составной бур-щит), wheel:* (моно-колесо «Канонир») и sprut:* (звено/лапа якорей «Спрута») — ГЛОБАЛЬНЫЕ ключи (render_borer/render_wheel/render_sprut читают ПЛОСКО из PART_SPRITES) → НЕ префиксовать корпусом; остальное — по типу корпуса.
        const glob = key.indexOf('borer:') === 0 || key.indexOf('wheel:') === 0 || key.indexOf('sprut:') === 0;
