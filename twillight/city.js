...
class City {
  ...
  // Шаг анимации меняющейся надписи фазы (slide+fade): фиксируем СМЕНУ фазы (старая надпись
  // станет «уезжающей»). render_city._drawBarText ТОЛЬКО читает эти поля (§6: draw не мутирует state).
  _stepBarText() {
    const phase = _cityPhase(this);
    if (this._barPhase !== phase) {
      if (this._barPhase != null) this._barPrev = this._barText;   // старая надпись уезжает вверх, новая выезжает снизу
      this._barPhase = phase;
      this._barT0 = performance.now();
    }
  ...
  _updateState(dt, atBase, siphoned, powered) {
    ...
    if (atBase) {
      ...
      // `siphoned` — рейдер активно высасывает реактор: дозарядка ПРИОСТАНОВЛЕНА (кража видна даже когда юнит дома).
      if (!siphoned && this.timer < this.timerMax) { this.timer = Math.min(this.timerMax, this.timer + this.recharge * dt); this.charging = true; }
