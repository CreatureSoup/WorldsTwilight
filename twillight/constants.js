...
const ECHO_WAVE_T = 0.42;         // сек: рост волны до полного радиуса
const ECHO_CD_BASE = 6.0;         // сек кулдаун эхо; трек «ВСПЫШКА ЭХО» уменьшает (negative step)
...
// ── ДЕЙСТВИЯ ЮНИТА (СТРОГОЕ разделение по СЛОТУ — НЕ ситуативно) ──────────────────────────────
// ГЛАВНОЕ действие — ВСЕГДА Пробел: слот БУРА (импульс-заряд удержанием, винт-запуск) + взаимодействие
//   (подтверждение печати, апгрейды у базы). ДОПОЛНИТЕЛЬНОЕ — ВСЕГДА Энтер: слот СКАНЕРА (радар-развёртка,
//   эхо-волна) + прочие гаджеты. Так бур и сканер НИКОГДА не дерутся за клавишу, независимо от сборки.
...
const KEY_SECONDARY = ['Enter', 'NumpadEnter'];    // дополнительное действие (сканер + гаджеты)
// Классификация активных модулей/свойств по действию: категория СЛОТА → действие. НОВЫЙ активный модуль → сюда.
const MODULE_ACTION = { drill: 'primary', aux: 'primary', scanner: 'secondary' };
