...
// ⚠️ PAL — ЕДИНСТВЕННЫЙ источник цвета. КОПИИ значений (синхронить ВРУЧНУЮ при правке!):
//   codex_dom.js (объект CX) · meta_dom.css (:root #metaScreen) · codex_dom.css (:root) — DOM-оверлеи.
// Полупрозрачные варианты — ТОЛЬКО через palRgba(PAL.x, a), НЕ вбивать rgba-триплеты руками (audit_2026-08).
// ============================================================
const PAL = {
  ...
  rust:   '#c0402f',                                                // износ/отказ/нехватка (был ФАНТОМОМ: `PAL.rust || '#c0402f'` в 5 местах работал только фолбэком — audit_2026-08)
  // семантические цвета сущностей (были размножены литералами по 6+ файлам — audit_2026-08)
  enemyEye: '#d0402f',   // красный глаз/сенсор ДИКИХ — общий знак всех врагов (канон: глаза всегда красные)
  enemyHot: '#ff6a4a',   // «горячее» состояние диких: бурение/атака/взлом — разогретый глаз
  screwGreen: '#9ad0a0', // зелень ВИНТОВОГО бура (модуль/щиты/кнопки действий — MODULE_DEFS.drill_screw.color тоже)
};
// rgba из hex-цвета PAL: ЕДИНСТВЕННЫЙ способ получить полупрозрачный вариант палитры.
const palRgba = (hex, a) => `rgba(${parseInt(hex.slice(1, 3), 16)},${parseInt(hex.slice(3, 5), 16)},${parseInt(hex.slice(5, 7), 16)},${a})`;
const TAU = Math.PI * 2;     // полный оборот; ⚠️ не вбивать 6.283 литералом (audit_2026-08)
...
// const POWERPLANT_RESERVE_CAP = 2;     // ×sec_per_organic: кап накопленного резерва (буфер плавности, чтобы не жечь по 1 в кадр)   // МЕРТВО (audit_2026-08: нигде не читается)
...
// ── СПРУТ «Ядро-якорник» (kind:'ring' + anchorLegs, sprut.js) — стартовые значения, тюнить в браузере ──
const SPRUT_HULL_HP = 100;            // как «Тор» (та же кольцевая платформа)
const SPRUT_REACH = 3.4;              // тайлов: дальность выстрела якоря от крепления
const SPRUT_OVERSTRETCH = 1.16;       // допуск растяжки (×reach) до мягкого перецепа; жёсткий срыв — ×1.35
const SPRUT_FIRE_T = 0.09;            // с: выдвижение якоря (выстрел)
const SPRUT_RETRACT_T = 0.14;         // с: втягивание щупальца
const SPRUT_REFIRE_CD = 0.24;         // с: личная пауза ноги между перецепами
const SPRUT_STAGGER = 0.07;           // с: глобальный интервал волны (ноги переставляются ПО ОЧЕРЕДИ, не разом)
const SPRUT_BEHIND_FRAC = 0.3;        // доля reach: якорь настолько «позади» по ходу → перецеп вперёд
const SPRUT_OPEN_HYST = 0.35;         // с: гистерезис переключения 8↔4 (анти-мерцание у кромок)
const SPRUT_LINK_SPACING = 11;        // design-px: шаг звеньев (`sprut:link`) вдоль щупальца
const SPRUT_FOLD_LEN = 13;            // design-px: длина СЛОЖЕННОЙ ноги (культя у крепления)
const SPRUT_BODY_SWAY = 3;            // design-px: кап смещения корпуса-подвеса на натянутых якорях
const SPRUT_LEGS_OPEN = 4;            // активных ног в ОТКРЫТОМ пространстве (≥3 сторон с >1 тайла воздуха)
const SPRUT_GAIT_PERIOD = 0.28;       // с: период волнового «ГРЕБКА» при движении — перецеп следующей ноги по кругу
const SPRUT_GAIT_BIAS = 0.6;          // сила наклона веера выстрела к направлению движения (0..1, микс векторов — ноги тянутся ВПЕРЁД)
const SPRUT_TRAIL_LEN = 24;           // design-px: длина ШЛЕЙФА ног ЗА корпусом в полёте (стражи из «Матрицы»)
const SPRUT_TRAIL_SPREAD = 0.34;      // рад: раскрытие веера шлейфа между ногами
const SPRUT_SQUAT = 6;                // design-px: присед корпуса на якорях в заряде прыжка (катапульта)
const SPRUT_KICK = 6;                 // design-px: импульс-выброс корпуса на отрыве / вжатие на ударе приземления (спад экспонентой)
const SPRUT_SWAY_AMP = 0.22;          // рад: амплитуда ПОИСКОВЫХ покачиваний сложенных ног (idle)
const SPRUT_PROBE_LEN = 0.5;          // доля reach: вылет холостого «ЩУПА» в пустоту (стоя, свободной ногой)
const SPRUT_PROBE_CD = 3.2;           // с: пауза между холостыми щупами (по одному за раз)
const SPRUT_SLIP_TIME = 0.5;          // с: сколько лапа «скребёт» НЕСТАБИЛЬНУЮ породу (t.unstable) до срыва-перецепа
const SPRUT_RAPPEL_LEN = 3;           // тайлов: длина «троса» рапеля — управляемый спуск с кромки/потолка (unit.js, только anchorLegs)
...
const HULL_DEFS = {
  ...
  // СПРУТ (kind:'ring' + anchorLegs): тело как у «Тора», но вместо IK-щупалец — 8 ПРЯМЫХ выстреливающих ЯКОРЕЙ
  // (локомоция-визуал — sprut.js; в открытом пространстве работают 4; мост/рапель — unit.js). ГЛАВНОЕ свойство —
  // ДВА доп-слота (`aux` + `aux2`; слот aux2 → категория 'aux' через inventory._slotCat). unlock — узел print_slots.
  sprut: { name: STR.hull.name.sprut, hp: SPRUT_HULL_HP, slots: ['drill', 'engine', 'scanner', 'cargo', 'aux', 'aux2'], optional: ['aux', 'aux2'], unlock: 'print_slots' },
};
...
const MODULE_DEFS = {
  ...
  drill_screw: { name: STR.module.name.drill_screw, category: 'drill', color: PAL.screwGreen, screw: 1, unlock: 'mast_ds' },
...
// ── ВРАГИ: БАЗОВЫЕ ТИПЫ (перенесено из enemy.js/projectiles.js — audit_2026-08: один домен — один реестр) ──
...
// снайперский выстрел (projectiles.js):
...
const ENEMY_FIRE_FLASH_T = 0.13;   // сек — вспышка выстрела у дула (снайпер/мортира; рендер читает e.firing>0)
const ENEMY_RECOVER_FRICTION = 0.88; // трение отскока в recover-фазе (охотник/таран, за кадр ×dt-цикла ai)
...
// const LURKER_REBURY_R = 4;      // куда перезакапывается после удара   // МЕРТВО (audit_2026-08: нигде не читается)
...
// const MENDER_SPEED = 4;         // скорость (тайла/с)   // МЕРТВО (audit_2026-08: нигде не читается)
...
// const HEAL_FX_INTERVAL = 0.4;    // сек между всплывающими «+» эффекта лечения (только на тиках реген HP)   // МЕРТВО (audit_2026-08: нигде не читается)
// const HEAL_FX_HP = 5;            // «+» лечения показываем за КАЖДЫЕ N восстановленных HP (а не по таймеру — иначе с трюмом «+» сыплется постоянно)   // МЕРТВО (audit_2026-08: нигде не читается)
...
const torusDist = (ax, ay, bx, by) => Math.hypot(wrapDeltaPx(ax, bx), ay - by);   // px-дистанция на торе; идиома hypot(wrapDeltaPx…) была вбита ×54 — новый код зовёт хелпер (audit_2026-08)
...
// const RAD_SOURCES = 4;                  // МЕРТВО (audit_2026-08): очаги теперь привязаны к артефактам (world.radSources), генератор удалён
const RAD_SOURCE_R = 20;                   // радиус влияния очага (тайлов): на краю фон ≈ 0
// const RAD_SOURCE_BAND = [120, 220];     // МЕРТВО (см. выше)
// const RAD_SOURCE_SPREAD = 45;           // МЕРТВО (см. выше)
...
// const MINE_BLINKS = 4;            // число вспышек за MINE_BLINK_T   // МЕРТВО (audit_2026-08: нигде не читается)
...
// Живучесть по роли (враги РАНЯТСЯ: турели/СВЧ/шипы/импульс/осада/канонир и др.). Охотник — таран-боец (танк);
...
const STRUCT_DEFS = {
  ...
  jammer:      { name: STR.structure.name.jammer,      b: 'jammer',    kind: 'active',  hp: 80,  build: 2.5, cost: { iron: 2, crystal: 2 },  color: PAL.screwGreen, energyMax: 100, radius: 4,   eRate: 4 },
...
const META_NAME = STR.metaCurrency.name;   // мета-валюта (банк save.meta); ТЕКСТ — в словаре lang_ru_constants (audit_2026-08: видимые строки не в constants)
const META_ABBR = STR.metaCurrency.abbr;
...
// const CABLE_BATT_R = 6;                             // print_batt: радиус ЛОКАЛЬНОГО питания от живой батареи (тайлы), отдельно от трейлинг-кабеля   // МЕРТВО (audit_2026-08: нигде не читается)
...
// (Палитра PAL — В НАЧАЛЕ файла: data-таблицы ниже (MODULE_DEFS и др.) читают её при загрузке.)
...
// ── ТЮНИНГ ПОДСИСТЕМ (перенесено из своих файлов — audit_2026-08: «новые числа — только сюда») ──
// радар-сканер (radar.js):
const RADAR_SMOOTH = 5;       // лаг доворота пеленга (1/сек) — «инерция»
const RADAR_SIG_SMOOTH = 3;   // сглаживание силы сигнала
const RADAR_JIT_FAR = 0.45;   // рад: рысканье пеленга при слабом сигнале
const RADAR_JIT_NEAR = 0.06;  // рад: остаточное рысканье у очага
const RADAR_SPAWN = 22;       // частиц/сек при полном сигнале
const RADAR_PSPEED = 1.7;     // норм.ед/сек к ядру (×R в рендере)
const RADAR_PLIFE = 1.2;      // сек — страховочный таймаут жизни частицы
const RADAR_SPREAD = 1.15;    // рад: полуразброс «прихода» при слабом сигнале (вблизи → к нулю)
// лут-дропы (loot.js):
...
// цикл волн (cycle.js) — ГЛАВНАЯ крутилка темпа мира:
