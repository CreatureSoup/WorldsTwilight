...
function drawTraps(ctx, game, camera) {
  ...
  if (game.seismicWaves) for (const w of game.seismicWaves) {
    ...
    ctx.beginPath(); ctx.arc(cx, cy, rad + band, 0, TAU); ctx.arc(cx, cy, Math.max(0, rad - band), 0, TAU, true); ctx.clip();
    const s = 1 + 0.11 * (1 - f);
    ...
    ctx.beginPath(); ctx.arc(cx, cy, rad, 0, TAU); ctx.stroke(); ctx.restore();
  }
}
